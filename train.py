...
from for_test import around_pad_seqence, trunaction
from my_model import Mymodel
...
def main():
  ...
  for i in tqdm(range(epoch)):
    ...
    # valid_loss, mean_valid_acc, valid_wer_record, valid_cer_record, result_table = validate(model, processor, valid_dataloader, criterion, device)
    # print('valid_loss', valid_loss, 'valid_wer', valid_wer_record, 'valid_cer', valid_cer_record)
    # if save_log:
    #   wandb.log({"valid_loss": valid_loss,
    #              "valid_acc": mean_valid_acc,
    #               "valid_wer": valid_wer_record,
    #               "valid_cer": valid_cer_record,
    #               "Teacher-force result": result_table},
    #             step=step_i)
    
    # torch.save({'model':model.state_dict(), 'optim':optimizer.state_dict()}, save_dir / f'val_best_wer_{i}.pt')
    # print("save model")
    
    # if (i+1) % 3 == 0:
    #   torch.save({'model':model.state_dict(), 'optim':optimizer.state_dict()}, save_dir / f'epoch_{i}.pt') # last epoch model save
    torch.save({'model':model.state_dict(), 'optim':optimizer.state_dict()}, save_dir / f'epoch_{i}.pt')
    # torch.save({'model':model.state_dict(), 'optim':optimizer.state_dict()}, save_dir / f'last_checkpoint.pt') 
