...
class RefTextEncoder(nn.Module):
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(torch.empty(n_ctx, n_in_state))
...
class TextRefDecoder(nn.Module):
    def __init__(
        self, n_vocab: int, n_ctx: int, n_state: int, n_head: int, n_layer: int, n_ref_layer: int
    ):
        super().__init__()
        ...
        self.ref_blocks: Iterable[ResidualAttentionBlock] = nn.ModuleList(
            [
                ResidualAttentionBlock(n_state, n_head, cross_attention=True)
    ...
    def forward(self, x: Tensor, xa: Tensor, xb: Tensor, kv_cache: Optional[dict] = None):
        """
        x : torch.LongTensor, shape = (batch_size, <= n_ctx)
            the text tokens
        ...
        for block in self.ref_blocks:
            x = block(x, xb, mask=self.mask, kv_cache=kv_cache)
...
class Mymodel(nn.Module):
    def __init__(self, dims: ModelDimensions):
        ...
        # )
        self.ref_encoder = RefTextEncoder(
            self.dims.n_vocab,
            self.dims.n_ref_text_ctx,
            self.dims.n_text_state,
            self.dims.n_ref_text_state,
            self.dims.n_text_head,
    ...
    def forward(
        self, mel: torch.Tensor, ref_text: torch.Tensor, tokens: torch.Tensor
        ...
        # return logits
        return self.decoder(tokens, self.encoder(mel), self.ref_encoder(ref_text))
